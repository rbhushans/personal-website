...
function fadeIn() {
    for (let i = 0; i < homeElements.length; i++) {
        let elem = homeElements[i]
        let dist = elem.getBoundingClientRect().top - window.innerHeight + 300;
        if (dist < 0) {
    ...
    for(let i = 1; i < projElements.length; i++){
        let elem = projElements[i]
        let dist = elem.getBoundingClientRect().top - window.innerHeight + 150;
        if (dist < 0) {
    ...
    for(let i = 1; i < expeElements.length; i++){
        let elem = expeElements[i]
        let dist = elem.getBoundingClientRect().top - window.innerHeight + 50;
        if (dist < 0) {
...
if(arrow != null){
    arrow.addEventListener('click', function(){
        ...
        let scrollContainer = document.getElementById("home-about");
        do {
